...
const Game: React.FC<SlotMachineProps> = ({ grid, isSpinning, data, winningLines, loadedImages, setLoadedImages }) => {
    ...
    const renderSidebar = (index: number) => {
        return (
            <img src={Sidebar} alt={("slot.bottomBar")} className={`h-[340px] w-2 md:w-6 -mb-4 ${index == 1 ? "scale-x-[-1]" : ""}`}
    ...
    const renderBottomBar = (index: number) => {
        return (
            ...
            <img src={bottomBar} alt={("slot.bottomBar")} className={`w-full md:w-[416px] aspect-[539/7] z-10 ${index == 0 ? "scale-y-[-1]" : ""} `}
                onLoad={() => setLoadedImages(loadedImages + 1)}
            />
        )
    }
                    ...
                    {[{
                        ...
                        (line, index) => (
                            <div className='flex relative ' key={index}>
                                ...
                                {
                                    index !== 2 && <img src={bar} alt={("slot.bar")} className={`
