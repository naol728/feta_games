...
export const isCollectionBadge = (key: string) =>
...
export async function getBadgeCatalog(): Promise<CatalogBadge[]> {
  const res = await apiClient.get("/users/badges/catalog");
...
export async function setWornBadge(
  badge: BadgeKey | null,
): Promise<BadgeChoice> {
  const res = await apiClient.put("/users/badge", { badge });
  return res.data;
}
...
export async function grantBadge(
  ...
  const res = await apiClient.put(`/admin/users/${userId}/badge`, {
    key,
    note,
    action,
  });
