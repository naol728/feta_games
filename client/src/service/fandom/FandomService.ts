...
export async function getBoards(
  ...
  const res = await apiClient.get("/fandom", {
    params: { sort, page, q: q || undefined },
  });
  return res.data;
}

export async function getBoard(name: string): Promise<FanBoard> {
  const res = await apiClient.get(`/fandom/${encodeURIComponent(name)}`);
  return res.data;
}
...
export async function getMyStanding(name: string): Promise<MyStanding> {
  const res = await apiClient.get(`/fandom/${encodeURIComponent(name)}/me`);
  return res.data;
}

export async function getReach(): Promise<ReachRow[]> {
  const res = await apiClient.get("/fandom/reach");
  return res.data.reach || [];
}

export async function getCollectors(): Promise<CollectionBoard> {
  const res = await apiClient.get("/fandom/collectors");
  return res.data;
}
